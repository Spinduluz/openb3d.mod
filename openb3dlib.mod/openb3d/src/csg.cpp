...
real npicked[3], tpicked[3];

real dist(real x, real y, real z, real x2, real y2, real z2){
...
real PointDistanceToLine(real ax, real ay, real az, real bx, real by, real bz, real px, real py, real pz){
	...
	real ab = sqrt( (bx-ax)*(bx-ax) + (by-ay)*(by-ay) + (bz-az)*(bz-az) );
	real bp = sqrt( (px-bx)*(px-bx) + (py-by)*(py-by) + (pz-bz)*(pz-bz) );
	real pa = sqrt( (ax-px)*(ax-px) + (ay-py)*(ay-py) + (az-pz)*(az-pz) );

	//get the triangle's semiperimeter
	real semi = (ab+bp+pa) / 2.0;
	
	//get the triangle's area
	real area = sqrt( semi * (semi-ab) * (semi-bp) * (semi-pa) );
...
CSGTriangle* MakeTriangle(Mesh* mesh, Surface* surf, real x0, real y0, real z0, real u0, real v0, real x1, real y1, real z1, real u1, real v1, real x2, real y2, real z2, real u2, real v2, int loop, CSGTriangle* org){

	//distance between corners
	real d1 = dist(x0,y0,z0, x1,y1,z1);
	real d2 = dist(x1,y1,z1, x2,y2,z2);
	real d3 = dist(x2,y2,z2, x0,y0,z0);
			
	//size of triangle
	real opp = PointDistanceToLine(x0,y0,z0,x1,y1,z1,x2,y2,z2);
	...
	real Ny = ( az * bx ) - ( ax * bz );
	real Nz = ( ax * by ) - ( ay * bx );
	real Ns = sqrt( Nx*Nx + Ny*Ny + Nz*Nz );
	if (Ns > 0){
		Nx = Nx / Ns;
		Ny = Ny / Ns;
		Nz = Nz / Ns;
...
void ScanObject(Mesh* mesh){
	...
	for (int si = 1; si<=mesh->CountSurfaces(); si++){
		...
		for (int t = 0; t<= s->CountTriangles() - 1; t++){
			...
			real v0 = s->TriangleVertex(t, 0);
			real v1 = s->TriangleVertex(t, 1);
			real v2 = s->TriangleVertex(t, 2);

			//get vertex coords			
			real v0x = s->VertexX(v0);
			real v0y = s->VertexY(v0);
			real v0z = s->VertexZ(v0);
			/*Entity::TFormPoint(v0x, v0y, v0z, mesh, 0);
			v0x = Entity::TFormedX();
			v0y = Entity::TFormedY();
			v0z = Entity::TFormedZ();*/

			real v1x = s->VertexX(v1);
			real v1y = s->VertexY(v1);
			real v1z = s->VertexZ(v1);
			/*Entity::TFormPoint(v1x, v1y, v1z, mesh, 0);
			v1x = Entity::TFormedX();
			v1y = Entity::TFormedY();
			v1z = Entity::TFormedZ();*/

			real v2x = s->VertexX(v2);
			real v2y = s->VertexY(v2);
			real v2z = s->VertexZ(v2);
			...
			real uu0 = s->VertexU(v0);
			real vv0 = s->VertexV(v0);
			real uu1 = s->VertexU(v1);
			real vv1 = s->VertexV(v1);
			real uu2 = s->VertexU(v2);
			real vv2 = s->VertexV(v2);
...
int ray_plane(real p1x, real p1y, real p1z, real p2x, real p2y, real p2z, real nx, real ny, real nz, real d){
...
int SplitTriangle(CSGTriangle* t1, CSGTriangle* t2, int dosplit = 0, int loop = 1){
	...
	real v0x = t1->x0; real v0y = t1->y0; real v0z = t1->z0;
	real v1x = t1->x1; real v1y = t1->y1; real v1z = t1->z1;
	real v2x = t1->x2; real v2y = t1->y2; real v2z = t1->z2;
	...
	real d1 = dist(v0x,v0y,v0z, ssx[0],ssy[0],ssz[0]); 	//distance from side 1
	real d2 = dist(ssx[0],ssy[0],ssz[0],v1x,v1y,v1z);  	//distance from side 2
	real dd = d1 + d2;					//total distance
	if (dd == 0) {
		ssu[0] = u0;
		ssv[0] = v0;
	}else{
	...
	if (stest[0]!=0 && stest[1]!=0 && stest[2]!=0){
		...
		if (dosplit!=0){
			...
			real d01 = dist(ssx[0],ssy[0],ssz[0], v0x,v0y,v0z);
			real d02 = dist(ssx[1],ssy[1],ssz[1], v0x,v0y,v0z);
			real d11 = dist(ssx[1],ssy[1],ssz[1], v1x,v1y,v1z);
			real d12 = dist(ssx[2],ssy[2],ssz[2], v1x,v1y,v1z);
			real d21 = dist(ssx[2],ssy[2],ssz[2], v2x,v2y,v2z);
			real d22 = dist(ssx[0],ssy[0],ssz[0], v2x,v2y,v2z);
			if (d02<d01)
				d01=d02;
			if (d12<d11)
				d11=d12;
...
real findmin(real a,real b,real c){
...
real findmax(real a,real b,real c){
...
bool BoxesOverlap(real x0, real y0, real z0, real w0, real h0, real d0, real x2, real y2, real z2, real w2, real h2, real d2){
...
bool CSGTrisIntersect(CSGTriangle* t1, CSGTriangle* t2){
	...
	//get coords t1
	real x0 = t1->x0; real y0 = t1->y0; real z0 = t1->z0;
	real x1 = t1->x1; real y1 = t1->y1; real z1 = t1->z1;
	real x2 = t1->x2; real y2 = t1->y2; real z2 = t1->z2;

	//get coords t2
	real tx0 = t2->x0; real ty0 = t2->y0; real tz0 = t2->z0;
	real tx1 = t2->x1; real ty1 = t2->y1; real tz1 = t2->z1;
	real tx2 = t2->x2; real ty2 = t2->y2; real tz2 = t2->z2;


	//determine min/max x/y/z values
	real nx1 = findmin(x0,x1,x2);
	real nx2 = findmax(x0,x1,x2);
	real ny1 = findmin(y0,y1,y2);
	real ny2 = findmax(y0,y1,y2);
	real nz1 = findmin(z0,z1,z2);
	real nz2 = findmax(z0,z1,z2);
	...
	//determine min/max x/y/z values
	real tnx1 = findmin(tx0,tx1,tx2);
	real tnx2 = findmax(tx0,tx1,tx2);
	real tny1 = findmin(ty0,ty1,ty2);
	real tny2 = findmax(ty0,ty1,ty2);
	real tnz1 = findmin(tz0,tz1,tz2);
	real tnz2 = findmax(tz0,tz1,tz2);
...
bool Ray_Intersect_Triangle(real Px, real  Py, real  Pz, real  Dx, real  Dy, real Dz, real V0x, real V0y, real V0z,
real V1x, real V1y, real V1z, real V2x, real V2y, real V2z, int Extend_To_Infinity=true, int Cull_Backfaces=false, int Flip_Faces=false){
	...
	real Hx = (Dy * E2z) - (E2y * Dz);
	real Hy = (Dz * E2x) - (E2z * Dx);
	real Hz = (Dx * E2y) - (E2x * Dy);

	// Calculate the dot product of the above vector and the vector between point 0 and point 2.
	real A = (E1x * Hx) + (E1y * Hy) + (E1z * Hz);
	...
	real Qx = (Sy * E1z) - (E1y * Sz);
	real Qy = (Sz * E1x) - (E1z * Sx);
	real Qz = (Sx * E1y) - (E1x * Sy);
	...
	real nx=(E1y*E2z)-(E1z*E2y);
	real ny=(E1z*E2x)-(E1x*E2z);
	real nz=(E1x*E2y)-(E1y*E2x);
	real d = -nx*V0x - ny*V0y - nz*V0z ;
	
	real denom = nx*Dx + ny*Dy + nz*Dz;
	real mu = - (d + nx*Px + ny*Py + nz*Pz) / denom;
...
real Ray_Intersect_Mesh_Max(Mesh* mesh, real Px, real Py, real Pz, real Dx, real Dy, real Dz, bool Extend_To_Infinity=true, bool Cull_Backfaces=false, bool Flip_Mesh=false, bool Method=1){
	...
	if (Surfaces > 0){
		...
		for (int SurfaceLoop = 1;SurfaceLoop<=Surfaces;SurfaceLoop++){
			...
			for (int TriLoop = 0; TriLoop<= Tris-1; TriLoop++){
				...
				real V0x = surface->VertexX(V0);
				real V0y = surface->VertexY(V0);
				real V0z = surface->VertexZ(V0);

				real V1x = surface->VertexX(V1);
				real V1y = surface->VertexY(V1);
				real V1z = surface->VertexZ(V1);

				real V2x = surface->VertexX(V2);
				real V2y = surface->VertexY(V2);
				real V2z = surface->VertexZ(V2);
				...
				if (Intersected){
					...
					real d = dist(Px, Py, Pz, tpicked[0], tpicked[1], tpicked[2]);
					if (d < max) 
						max = d;
					count++;
	...
	for(int si=1;si<=mesh->CountSurfaces();si++){
		...
		for(ic=CSGTriangle::CSGTriangle_list.begin();ic!=CSGTriangle::CSGTriangle_list.end();ic++){
			...
			if (c->surf==surf){
				...
				real x0 = Entity::TFormedX();
				real y0 = Entity::TFormedY();
				real z0 = Entity::TFormedZ();

				Entity::TFormPoint(c->x1,c->y1,c->z1, 0, mesh);
				real x1 = Entity::TFormedX();
				real y1 = Entity::TFormedY();
				real z1 = Entity::TFormedZ();

				Entity::TFormPoint(c->x2,c->y2,c->z2, 0, mesh);
				real x2 = Entity::TFormedX();
				real y2 = Entity::TFormedY();
				real z2 = Entity::TFormedZ();*/
				...
				if (!((opp < ep) || (d1<ep) || (d2<ep) || (d3<ep))) {
					...
					//get middle point				
					real mx = (c->x0+c->x1+c->x2) / 3.0;
					real my = (c->y0+c->y1+c->y2) / 3.0;
					real mz = (c->z0+c->z1+c->z2) / 3.0;
					...
					;;METHOD 2*/
					real d1 = Ray_Intersect_Mesh_Max(dummy, mx, my, mz, -c->nx, -c->ny, -c->nz, true, true, false);
					real d2 = Ray_Intersect_Mesh_Max(dummy, mx, my, mz, -c->nx, -c->ny, -c->nz, true, true, true);
					int finside = (d1 <= d2);
					real time;
